            ...
            for i, ob_main in enumerate(objects):
                ...
                for ob, ob_mat in obs:
                    with ProgressReportSubstep(subprogress1, 6) as subprogress2:
                        ...
                        if EXPORT_NORMALS:
                            loops_to_normals = [0] * len(me_verts)
                            #for v_index in range(0,len(me_verts)):
                                #loops_to_normals[v_index] = v_index
                                #fw('vn %.6f %.6f %.6f\n' % me_verts[v_index].normal[:])
                                #no_unique_count += 1
                        else:
                            loops_to_normals = []
                        ...
                        for f, f_index in face_index_pairs:
                            ...
                            f_v_loop_i = 0
                            for vi, v, li in f_v:
                                pixel_a = (v.index, uv_face_mapping[f_index][vi])
                                pixel_get = pixel_dict.get(pixel_a)
                                if pixel_get is None:
                                    pixel_list.append(pixel_a)
                                    pixel_dict[pixel_a] = pixel_count
                                    face_a[f_v_loop_i] = pixel_count
                                    pixel_count += 1
                                else:
                                    face_a[f_v_loop_i] = pixel_get
                                f_v_loop_i += 1
                            face_list.append(face_a)

                        subprogress2.step()

                        for pi in pixel_list:
                            fw('v %.6f %.6f %.6f\n' % me_verts[pi[0]].co[:])
                        fw('\n')
                        for pi in pixel_list:
                            fw('vt %.6f %.6f\n' % uv_list_original[pi[1]][:])
                        fw('\n')
                        for pi in pixel_list:
                            fw('vn %.6f %.6f %.6f\n' % me_verts[pi[0]].normal[:])
                        fw('\n')

                        for face_a in face_list:
                            fw('f')
                            for verind_a in face_a:
                                fw(" %d/%d/%d" % (totverts + verind_a,
                                    totuvco + verind_a,
                                    totno + verind_a,
                                    ))  # vert, uv, normal
                            fw('\n')
                        ...
                        totverts += len(pixel_list)
                        totuvco += len(pixel_list)
                        totno += len(pixel_list)
